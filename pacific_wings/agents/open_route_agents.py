"""
Multi-agent narrative layer for new/candidate-route analysis (analysis/open_route.py).
...
def risk_commentary(analysis: dict) -> dict:
    ...
    # Only the sections that exist: a route beyond the fleet's range returns
    # no risk or financial sections at all, deliberately.
    payload = {
        key: analysis[key]
        for key in ("route", "risk", "financials", "operations")
        if key in analysis
    }
    user_message = (
