...
# Committed starting content. reports.json itself is gitignored: the app writes
# to it, so tracking it made every ordinary run show up as an uncommitted diff.
SEED_PATH = ROOT / "data" / "reports.seed.json"

_lock = Lock()


def _load() -> list[dict]:
    for path in (REPORTS_PATH, SEED_PATH):
        if path.exists():
