> **Historical document.** This is the roadmap the project was built against,
> kept because the reasoning behind the three-tier data model is still worth
> reading. **Its figures and module paths are superseded** — several of the
> results claimed below were later found to be artifacts and were rebuilt.
>
> For what the system actually does now:
> [`calculations_and_models.md`](calculations_and_models.md) for the methodology,
> [`model_metrics.md`](model_metrics.md) for live model scores (regenerated by
> every training run), and [`data_methodology.md`](data_methodology.md) for
> provenance field by field.
>
> The most important divergence: the demand model described here predicts
> Pacific Wings' own passengers from a pre-capped label. That was the defect
> that made every strategy lever inert, and the target is now the total route
> market. See §1 of the methodology doc.
...
9. Phase 3: `pacific_wings/ml/features.py`, `pacific_wings/ml/train.py` (XGBoost, time-based 2022-2023 train / 2024 test split),
   `pacific_wings/api/` (`/demand_forecast`, `/health`) — verified end-to-end via local server. Training target was
...
10. Phase 4-5: `data/reference/fuel_prices.csv` (curated EIA annual averages), `docs/cost_assumptions.md`,
    `pacific_wings/simulation/revenue.py` (cabin fare split + ancillary), `pacific_wings/simulation/cost.py` (fuel/non-fuel CASM split,
    what-if fuel price lever), `pacific_wings/api/` `/route_economics` (demand -> revenue -> cost -> profit) —
    verified end-to-end including a fuel +30% what-if scenario
11. Phase 6-7: `pacific_wings/simulation/market_share.py` (multinomial logit over Pacific Wings + synthetic competitors),
    `pacific_wings/simulation/engine.py` (`SimulationEngine.run_scenario`/`compare` - applies price/frequency/aircraft/
    fuel/rating deltas, caps passengers carried by capacity, recomputes revenue/cost/profit/market share),
    `pacific_wings/api/` `/what_if` — verified with fare-cut, frequency-increase, and aircraft-swap scenarios
12. Phase 8-9: `agents/` — LangGraph `StateGraph` (`pacific_wings/agents/graph.py`) wiring Demand and Finance agents
    (pure extraction from `SimulationEngine.compare()`, no LLM) and Market/Risk/Strategy agents (Gemini via
    `pacific_wings/agents/llm_client.py`, `gemini-2.5-flash` free tier, grounded in real macro/tourism data +
    calibrated-synthetic competitor data via `pacific_wings/agents/context.py`). `pacific_wings/agents/copilot.py` orchestrates the graph
    and produces an executive summary; `pacific_wings/api/` `/copilot` — verified end-to-end including graceful
...
13. Phase 10: `pacific_wings/simulation/presets.py` (`fuel_price_shock`, `tourism_boom`, `competitor_entry` - thin wrappers
    deriving scenario kwargs from route reference data), extended `pacific_wings/ml/features.py` (`tourism_arrivals_multiplier`,
    `extra_competitors`), `pacific_wings/simulation/market_share.py` (`extra_competitors`), and `pacific_wings/simulation/engine.py` to
    thread both levers through `run_scenario`/`compare`. `pacific_wings/api/` `/what_if_presets` (lists presets) and
...
14. Phase 11: `frontend/` - Next.js (App Router, TypeScript, Tailwind, Recharts, React-Leaflet@5) dashboard with
    ...
    new `pacific_wings/api/` `GET /routes` endpoint merging `airline_profile.json` with `airports.csv` coordinates,
...
15. Phase 12: `Dockerfile` (backend - copies pre-trained model + reference/profile data, runs
    `uvicorn pacific_wings.api.main:app`), `frontend/Dockerfile` (multi-stage Next.js standalone build,
...
16. **Real-data rebuild** (post-launch hardening — replacing the synthetic-but-calibrated tier with real,
    ...
    - Phase 2 — real competitors: `etl/generate_synthetic_demand.py`'s `COMPETITORS` now lists real carriers/
      frequencies/Skytrax ratings/spot-checked fares per route; recalibrated `pacific_wings/simulation/market_share.py`'s
    ...
    - Phase 3 — real demand: new `etl/fetch_real_aviation_stats.py` parses two manually-downloaded BITRE
      ...
      (documented choice, not an oversight). Retrained `pacific_wings/ml/train.py` — see item 9 for the
    ...
    - Phase 4 — cost & fare calibration: `data/aircraft_specs.json`'s non-fuel CASM is now anchored to Qantas
      ...
      `pacific_wings/analysis/open_route.py`'s separate fare-by-distance table (used for arbitrary new destinations)
    ...
    - Phase 5 — forecasting research deepening: `pacific_wings/ml/train.py` adds 5-fold cross-validation
      ...
      `pacific_wings/simulation/monte_carlo.py` samples fuel price and GDP growth from REAL historical volatility
...
17. **Standalone Open Route page**: the worldwide new-route analysis (`pacific_wings/analysis/open_route.py` +
    `pacific_wings/agents/open_route_agents.py`) was previously chat-only. Added `pacific_wings/api/`'s `GET /analyze_route_agents`
...
18. **Frontend realism audit** (3 Explore agents traced every displayed number on every page back to a real
    ...
    **Phase B (replace fabrications with real backend data that already exists) — DONE (2026-06-18)**:
    ...
    `/what_if` — added an optional `fuel_price_center` param to `pacific_wings/simulation/monte_carlo.py`/the endpoint so
    ...
    `pacific_wings/analysis/open_route.py`'s existing real risk tables via `pacific_wings/agents/context.py`) instead of a
    ...
    **Confidence score follow-up — DONE (2026-06-19).** User asked specifically to reintroduce a real
    ...
    rather than fabricating one. New `pacific_wings/ml/confidence.py` (`ConfidenceModel`) combines three real signals: (1)
    a 30-model bootstrap ensemble (`pacific_wings/ml/train.py` now also trains resample-with-replacement
